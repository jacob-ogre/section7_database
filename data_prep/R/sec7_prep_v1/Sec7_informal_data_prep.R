# Sec7_informal_summery.R
# Summary stats and figures for Sec7 informal consultations.
...
# loading.
dat <- read.table("informal_2008-2014_uniq_filled.tab",
                  stringsAsFactors=FALSE,
                  header=TRUE,
                  sep="\t")
save(dat, 
     file="informal_2008-2014.RData")
rm(dat)

# After initial read, just load the RData (faster, smaller size)
load("informal_2008-2014.RData")
names(dat)
...
# Re-save the data 
save(dat, file="informal_2008-2014_cleaned.RData")
...
dat2 <- subset(dat2, dat2[,11] <= dat2[,13])
dim(dat2)

#-----------------------
# Write the trimmed data to tab'd file for 
write.table(dat2,
            file="informal_2008-2014_cleaned_datecut.tab",
            sep="\t",
            row.names=FALSE,
            quote=FALSE)
save(dat2, file="informal_2008-2014_cleaned_datecut.RData")
...
infile <- "informal_2008-2014_cleaned_datecut.tab"
outfil <- "inform_compressed.tab"
...
###############################################################################
rm(list=ls())
dat <- read.table("inform_compressed.tab",
                  sep="\t",
                  header=TRUE,
                  stringsAsFactors=FALSE)
...
# Re-save the data 
save(dat, file="inform_compressed.RData")
save(dat2, file="informal_2008-2014_cleaned.RData")
rm(list=ls())
